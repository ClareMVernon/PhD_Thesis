---
title: 'S8.1. Code used to undertake systematic review of shrub response to drivers. '
author: "Clare Vernon"
date: "2025-03-11"
output:
...
Package for drawing NSW / Victorian borders: 
...
df_mapcount <- df %>%
  ...
  summarise(Count = n_distinct(UniqueCode), Lat = unique(coord.x.map), Long = unique(coord.y.map)) # How many studies per location? 
df_mapcount
...
fig_1 <- p1 | region_map | p2
fig_1 
```

### 2.2. Count of number of reports, for each year of publication.
```{r}
...
### 3.2. Figure 2. Count of studies reporting a promoting, reducing and no effect for all driving processes, and for driving processes grouped under climate change.
...
p_all <- df_y_neg2 %>% 
  ...
  ylab("Count of studies") + xlab("Driver") +
...
p_cc <- df_y_neg %>% 
  ...
  ggtitle("Drivers grouped under 'Climate change'") +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 20)) +
  ylab("Count of studies") + xlab("Driver")
p_cc  

# Combining both into Figure 2. 
p_driver <- p_all | p_cc 
fig_2 <- p_driver  + plot_layout(guides = 'collect', axes = "collect", widths = c(1,1)) + plot_annotation(tag_levels = 'a') 
fig_2
...
### 3.3. Figure 3. Count of studies reporting a promoting, reducing and no effect of a driving process when assessed at different population scales, and using different metrics of change.
...
p_metric <- df_y_neg2 %>% 
  ...
  theme(        panel.spacing = unit(2, "lines")) + guides(fill=guide_legend(title="Direction of change")) + ylab("Count of studies") + xlab("Driver") + ylim (-20, 20) +
  ggtitle("Metric used to measure change") +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 20))
p_metric
...
p_poplevel <- df_y_neg %>% 
  ...
  theme(        panel.spacing = unit(2, "lines")) + guides(fill=guide_legend(title="Direction of change")) + ylab("Count of studies") + xlab("Driver") + ylim (-20, 20) +
  ggtitle("Scale of shrub population assessed") +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 20))
p_poplevel  

# Combining both into Figure 3. 
p_metric_pop <- p_metric / p_poplevel  
fig_3 <- p_metric_pop + plot_layout(guides = 'collect', axes = "collect", widths = c(1,1)) + plot_annotation(tag_levels = 'a') 
fig_3
```

### 3.4. Test of significant differences using chi-squared. 

```{r}
# Population scale, direction of change
test <- chisq.test(table(df$Response.Level, df$QualDirection))
test
...
# Response metric, direction of change
test <- chisq.test(table(df$Metric.Aggregate, df$QualDirection))
test

# Ecosystem type, direction of change
test <- chisq.test(table(df$Aggregate.InitialES, df$QualDirection))
test

# Direction of change when shrubs, assessed at species population scale, are grouped by alpine zone
count <- df %>%
  filter(Response.Level == "Species") %>%
  group_by(Response.Level, Alpine.Zone) %>%
  summarise(Count = n_distinct(UniqueCode))
count
test <- chisq.test(table(df$Alpine.Zone, df$QualDirection))
test

# # Direction of change when shrubs, assessed at species population scale, are grouped by groundwater dependence
count <- df %>%
  filter(Response.Level == "Species") %>%
  group_by(Response.Level, `Groundwater Association`) %>%
  summarise(Count = n_distinct(UniqueCode))
count
test <- chisq.test(table(df$`Groundwater Association`, df$QualDirection))
test

# # Direction of change when shrubs, assessed at species population scale, are grouped by trait
count <- df %>%
  filter(Response.Level == "Species") %>%
  group_by(Response.Level, Seeder.Resprouter) %>%
  summarise(Count = n_distinct(UniqueCode))
count
test <- chisq.test(table(df$Seeder.Resprouter, df$QualDirection))
test

```

**Outcome of chi-squared tests:** No significant differences in reported outcome and the metric of change used. Significant differences between population scale and direction of change, population scale and driving process assessed. Species level responses did not differ between alpine zones or groundwater dependence, but did differ between traits.

### 3.5. Appendix S2. Direction of change per ecosytem type in response to driver.
...
p_ES # Figure presented in S5

test <- chisq.test(table(df$Aggregate.InitialES, df$QualDirection))
test

# Does the number of studies reporting a promoting, reducing or no effect differ between ecosystems which assessed climate change? 
unique(df$Treatment_Thematic)
df_climatechange <- df %>% filter (Treatment_Thematic == "Climate change")
test <- chisq.test(table(df_climatechange$Aggregate.InitialES, df_climatechange$QualDirection))
test

# How many studies report a promoting, reducing, or no effect of fire on shrub abundance for each ecosystem type? 
df_fire_ecosystem <- df %>%
  filter(Treatment_Thematic ==  "Fire") %>%
  group_by(Treatment_Thematic, Aggregate.InitialES, QualDirection) %>%
  summarise(Count = n_distinct(UniqueCode)) # Filter by metric
df_fire_ecosystem
